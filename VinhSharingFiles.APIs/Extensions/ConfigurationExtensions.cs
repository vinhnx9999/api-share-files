    ...
    public static class ConfigurationExtensions
    {
        ...
        public static AWSOptions GetAWSOptions(this IConfiguration config, string configSection)
        {
            ...
            foreach (IConfigurationSection element in configuration.GetChildren())
            {
                try
                {
                    PropertyInfo propertyInfo = typeInfo.DeclaredProperties.SingleOrDefault((PropertyInfo p) => p.Name.Equals(element.Key, StringComparison.OrdinalIgnoreCase));
                    if (!(propertyInfo == null) && !(propertyInfo.SetMethod == null))
                    {
                        ...
                        {
                            object obj = Convert.ChangeType(element.Value, propertyInfo.PropertyType);
                            propertyInfo.SetMethod.Invoke(aWSOptions.DefaultClientConfig, new object[1] { obj });
                        }
                        else if (propertyInfo.PropertyType == typeof(TimeSpan) || propertyInfo.PropertyType == typeof(TimeSpan?))
                        {
                            ...
                            propertyInfo.SetMethod.Invoke(aWSOptions.DefaultClientConfig, new object[1] { timeSpan });
                        }
                    }
                }
